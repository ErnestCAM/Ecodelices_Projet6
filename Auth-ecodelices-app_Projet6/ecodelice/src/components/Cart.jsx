  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      navigate(`/payment/${orderId}`, { 
        state: { basket: cartItems, total, orderId } 
      });
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 py-20 px-4 text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <a href="/products" className="bg-green-600 text-white px-12 py-4 rounded-xl font-bold hover:bg-green-700">
            Voir les produits
          </a>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-4xl font-bold text-gray-900 mb-12 text-center">Votre panier</h1>
        
        <div className="grid lg:grid-cols-3 gap-8">
          {/* Articles */}
          <div className="lg:col-span-2 space-y-6">
            {cartItems.map((item) => (
              <div key={item.id} className="bg-white p-6 rounded-2xl shadow-lg flex items-center gap-6">
                <img src={item.image || '/placeholder.jpg'} alt={item.name} className="w-24 h-24 object-cover rounded-xl" />
          ...
          <div className="bg-white p-8 rounded-3xl shadow-xl sticky top-8">
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-green-600 text-white py-4 px-6 rounded-2xl font-bold text-xl hover:bg-green-700 disabled:opacity-50"
            >
